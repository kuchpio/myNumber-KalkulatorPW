...
char *numberToChars(myNumber *number) //converts myNumber to string 
{
    ...
    int firstPrintableIndex = numberSize - 1;
    char *result;
    ...
    while (MNgetDigit(number, firstPrintableIndex) == 0 && firstPrintableIndex >= 0) 
        firstPrintableIndex--;

    //allocate memory for new number
    result = malloc((firstPrintableIndex + 2) * sizeof(char));

    //shift values to their chars
    for (int j = 0; j <= firstPrintableIndex; j++)
    {
        digitValue = MNgetDigit(number, firstPrintableIndex - j);
        ...
        {
            printf("Incorrect value: %d (index: %d)\n", digitValue, firstPrintableIndex - j);
        }
    }

    *(result + firstPrintableIndex + 1) = '\0';
...
int MNcompare(myNumber *a, myNumber *b) //returns 1 when a > b, 0 otherwise
{
    ...
    while (MNgetDigit(a, aSize - 1) == 0) aSize--;
    while (MNgetDigit(b, bSize - 1) == 0) bSize--;
...
int MNdivide(myNumber *dividend, myNumber *divisor, myNumber *quotient, myNumber *residue, unsigned char base) {
    ...
    int dividendIsZero = 1, residueIsNULL = 0;

    for (int i = 0; i < divisorSize; i++) 
    {
        if (MNgetDigit(divisor, i) != 0)
            dividendIsZero = 0;
    }
...
int MNraise(myNumber *a, myNumber *exponent, myNumber *result, unsigned char base) 
{
    ...
    myNumber *temp = MNinit(MNsize(result));
